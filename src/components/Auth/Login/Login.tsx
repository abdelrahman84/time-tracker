...
function Login() {
  ...
  const handleLogin = async (values: EmailValue): Promise<void> => {
    await AuthApi.checkEmailBeforeLogin(values.email)
      .then((response) => {
    ...
    await AuthApi.forgotPasswordEmail(email)
      .then((response) => {
        toast({
          title: 'Success',
          description:
            'If the email exists, a reset password link will be sent to your email',
          status: 'success',
          duration: 3000,
          position: 'bottom-right',
        });
  ...
  const cardContent = (): React.ReactNode => {
    if (showPasswordForm) {
      return (
        <PasswordForm
          onHandlePassword={handlePasswordSubmit}
