...
function TimerDisplay(props: TimerDisplayProps) {
  ...
  const [remainingLoops, setRemainingLoops] = useState(
    Number(props.remainingLoops ?? 1),
  );
  const [localMinutes, setLocalMinutes] = useState(
    Number(props.remainingMinutes ?? 0),
  );
  const [localHours, setLocalHours] = useState(
    Number(props.remainingHours ?? 0),
  );
  const [localSeconds, setLocalSeconds] = useState(
    Number(
      props.remainingSeconds > 0 ? props.remainingSeconds : DEFAULT_SECONDS,
    ),
  );
  const targetEndTimeRef = React.useRef<number | null>(null);
  const [isTimerFinishedModalOpen, setIsTimerFinishedModalOpen] =
    useState(false);
  ...
  const updateCurrentCountDown = (): void => {
    ...
    if (diff === 0) {
      if (remainingLoops > 1) {
        ...
        const newEndTime =
          Date.now() + props.minutes * 60000 + props.seconds * 1000 + 1000;
  ...
  return (
    <Container>
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Timer countdown finished</ModalHeader>
          <ModalCloseButton />
        </ModalContent>
      </Modal>
