...
import {
  ...
  setMinutes,
  setSeconds,
} from 'state/TimerCountdownReducer';
...
function TimerDashboard() {
  const seconds = useSelector(
    (state: RootState) => state.timerCountdown.seconds,
  );
  const minutes = useSelector(
    (state: RootState) => state.timerCountdown.minutes,
  );
  const hours = useSelector((state: RootState) => state.timerCountdown.hours);
  const loopCount = useSelector(
    (state: RootState) => state.timerCountdown.loopCount,
  );
  const isLoopEnabled = useSelector(
    (state: RootState) => state.timerCountdown.isLoopEnabled,
  );
  ...
  return (
    <Container className={styles.timerDashboard}>
      ...
      {selectedType === STOPWATCH && timerEnabled === false && (
        <TimerStopWatch onTimerTypeChange={handleTimerTypeChange} />
      )}
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Previous Timer Found</ModalHeader>
          <ModalCloseButton />
          <ModalBody>{previousTimerContent()}</ModalBody>
